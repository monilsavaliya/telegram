...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if intent in ["GENERAL", "GREETING", "FLIRT", "CHAT"]:
        ...
        system_prompt = (
            ...
            f"ADAPTIVE: {persona['instruction']}\n"
            f"STYLE: {persona['style']}\n"
            "PRIME DIRECTIVE 1: Be intensely curious about the user, but keep it CHILL. Don't be over-enthusiastic.\n"
            "PRIME DIRECTIVE 2: Use emojis naturally (1-2 max per message). Don't spam them.\n"
            "EXCEPTION: If user says 'Goodnight', 'Bye', 'Sleep', 'Chhod' -> DO NOT ask a question. Just say meaningful final words.\n"
            f"AVOID: {avoids}\n"
            f"HISTORY:\n{history_str}\n"
            f"Jarvis:"
        )
